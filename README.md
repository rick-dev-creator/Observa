...
- **Verify before claiming.** The LLM does not say "this works" without showing test output, a curl response, or a build log. When the owner spotted that the running balance reported ~$80k against a real ~$60k Patreon lifetime, the fix was to validate the API math against ground-truth `lifetime_support_cents` rather than guess at the cause.
